...
| `x` | T13 | ~~Remove dev-only linter packages from the `ci` devShell~~ (superseded by T17) |
...
14. **`better-sqlite3` 11.x built against Node 24**: cavemem 0.2.1 declares `better-sqlite3: ^11.5.0`, which resolves to 11.10.0 -- a release line upstream does not test against Node 24 (`NODE_MODULE_VERSION` 137). The `stop` hook has aborted at process teardown inside the addon (`Assertion failed: (env) != nullptr`, `node::RemoveEnvironmentCleanupHook` at `src/api/hooks.cc:142`, reached from `Statement::~Statement()` under a GC weak callback). Mitigated, not proven fixed: the local `package.json` override pins 12.11.1, the line upstream supports on Node 24. `Statement::~Statement()` is byte-identical in 11.10.0 and 12.11.1, both addons import `node::RemoveEnvironmentCleanupHook`, and the abort did not reproduce in 35 runs of the 11.10.0 build against an empty and a 154 MB store, so the crash may need an upstream fix in cavemem's shutdown path.
