...
14. **Generated lefthook configuration omitted `actionlint`**: The `actions` fragment was declared but the materialized `lefthook.yml` was stale after the fragment update. Fixed by regenerating the hook configuration with the `actionlint` pre-commit and pre-push commands.
15. **Generated lefthook configuration was out of sync with its source fragments**: `lefthook.yml` contained `actionlint` hooks, but `flake.nix` omitted the `actions` fragment, so the guardrail fidelity check rejected the generated configuration. Fixed by declaring the `actions` fragment in `flake.nix`.
