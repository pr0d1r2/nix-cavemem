...
| status | id | goal |
| --- | --- | --- |
| ` ` | T11 | Inline the `ci` devShell as its own `pkgs.mkShell` instead of aliasing `default` (§B6; `flake.nix:114` `ci = default`) — duplicate the full package list so each shell can be trimmed independently |
